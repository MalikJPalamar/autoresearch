...
if HAS_CUDA:
    ...
    def make_dataloader(tokenizer, B, T, split, buffer_size=1):
        row_capacity = T + 1
        ...
        out_inputs = torch.empty((B, T), dtype=torch.long)
        out_targets = torch.empty((B, T), dtype=torch.long)
        while True:
            for row_idx in range(B):
                ...
                while pos < row_capacity:
                    ...
                    if best_idx >= 0:
                        ...
                        pos += remaining
            out_inputs.copy_(row_buffer[:, :-1])
            out_targets.copy_(row_buffer[:, 1:])
            yield out_inputs, out_targets, epoch

    @torch.no_grad()
    def evaluate_bpb(model, tokenizer, batch_size):
        token_bytes = _get_token_bytes(device="cpu")
        val_loader = make_dataloader(tokenizer, batch_size, MAX_SEQ_LEN, "val")
        steps = min(EVAL_TOKENS // (batch_size * MAX_SEQ_LEN), 20)
...
class GPT(nn.Module):
    ...
    def _precompute_rotary_embeddings(self, seq_len, head_dim, base=3000, device=None):
        if device is None:
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=x0_params, lr=scalar_lr, betas=(0.5, 0.99), eps=1e-10, weight_decay=0.0),
        ]
        for shape in sorted({p.shape for p in matrix_params}):
            group_params = [p for p in matrix_params if p.shape == shape]
            param_groups.append(dict(
                kind='muon', params=group_params, lr=matrix_lr,
                momentum=0.95, ns_steps=1, beta2=0.99, weight_decay=weight_decay,
            ))
...
# Optimization
TOTAL_BATCH_SIZE = 2**19 if HAS_CUDA else 2**19 # tokens per optimizer step
EMBEDDING_LR = 0.7      # learning rate for token embeddings (Adam)
UNEMBEDDING_LR = 0.006  # learning rate for lm_head (Adam)
MATRIX_LR = 0.20        # learning rate for matrix parameters (Muon)
...
STABLE_RATIO = 0.35     # fraction of time budget at peak LR (WSD schedule)
...
def build_model_config(depth):
    ...
    return GPTConfig(
        ...
        window_pattern=WINDOW_PATTERN, dropout=0.2, attn_dropout=0.08, mlp_dropout=0.25,
    )
...
peak_vram_mb = torch.cuda.max_memory_allocated() / 1024 / 1024 if HAS_CUDA else 0
