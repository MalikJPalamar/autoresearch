...
from prepare import MAX_SEQ_LEN, TIME_BUDGET, Tokenizer, EVAL_TOKENS
if USE_CUDA:
    from prepare import make_dataloader, evaluate_bpb
else:
    from prepare import _document_batches, get_token_bytes
    ...
    CPU_EVAL_TOKENS = 2 * 524288

    @torch.no_grad()
    def evaluate_bpb(model, tokenizer, batch_size):
        token_bytes = get_token_bytes(device="cpu")
        val_loader = make_dataloader(tokenizer, batch_size, MAX_SEQ_LEN, "val")
        steps = CPU_EVAL_TOKENS // (batch_size * MAX_SEQ_LEN)
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        # Embedding (tied with unembedding) — moderate init for both roles
