...
class Block(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        h = norm(x)
        x = x + self.attn(h, ve, cos_sin, window_size) + F.dropout(self.mlp(h), p=self.dropout, training=self.training)
        return x
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        torch.nn.init.normal_(self.transformer.wte.weight, mean=0.0, std=0.2)
        torch.nn.init.normal_(self.lm_head.weight, mean=0.0, std=0.0001)
        # Transformer blocks
...
DROPOUT = 0.12                              # dropout rate for regularization
